...
Loop
	...
	label_A
		...
		tblrd*-
		tblrd*-
		tblrd*-			;change pointer to 7 steps back, points to x[n-5]
		tblrd*-
		tblrd*-
		tblrd*-
		tblrd*-
		movf TABLAT, W 	;TABLAT is the value that corresponds to x[n-5]

		movwf TEMP
	
		addwf value,W ; x[n]+x[n-5]
		movwf SUML ;result into SUML
		
		movf TEMPH,W
		
		addwfc valueH,w ;x[n]+x[n-5] with bit carry
...
;
SimpleTable 
db 250,200,150,100,50,0,50,100,150,200,250,200,150,100,50,0
; --------------------------------------------------------------
