...
Loop
	...
	label_A
		...
		tblrd*-
		tblrd*-
		tblrd*-			;change pointer to 3 steps back, points to x[n-1]
		movf TABLAT, W 	;TABLAT is the value that corresponds to x[n-1]

		movwf TEMP
	
		addwf value,W ; x[n]+x[n-1]
		movwf SUML ;result into SUML
		
		movf TEMPH,W
		
		addwfc valueH,w ;x[n]+x[n-1] with bit carry
...
;
SimpleTable 
db 50,0,50,100,150,200,250,200,150,100,50,0
; --------------------------------------------------------------
