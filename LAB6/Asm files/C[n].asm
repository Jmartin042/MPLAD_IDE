...
Loop
	...
	label_A
		...
		movf xn1, W ; changes wreg to xn31
		
		addwf value,W ; x[n]+x[n-1]=SUML
		movwf SUML ;result into SUML
		
		movf TEMPH,W

		addwfc valueH,w ;x[n]+x[n-1]=SUMH with bit carry
		movwf SUMH ;result into SUMH
		
		;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;Second Summation;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		
		movf xn8,W ;changes wreg to xn8
		
		addwf xn9,W ; x[n-8]+x[n-9]=SUM2L
		movwf SUM2L ;result into SUM2L
		
		movf TEMPH,W

		addwfc valueH,w ;x[n-8]+x[n-9]=SUM2H with bit carry
