...
Loop
	...
	label_A
		...
		movf xn2, W ; changes wreg to xn2
		
		addwf value,W ; x[n]+x[n-2]=SUML
		movwf SUML ;result into SUML
		
		movf TEMPH,W

		addwfc valueH,w ;x[n]+x[n-2]=SUMH with bit carry
		movwf SUMH ;result into SUMH
		
		;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;Second Summation;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		
		movf xn4,W ;changes wreg to xn4
		
		addwf xn6,W ; x[n-4]+x[n-6]=SUM2L
		movwf SUM2L ;result into SUM2L
		
		movf TEMPH,W

		addwfc valueH,w ;x[n-4]+x[n-6]=SUM2H with bit carry
